test_all () {
  ...
  for test_file in $(find testcases -name "*.html")  # $test_file is like testcases/foo.html
  do
    test_base=${test_file%.html}                  # like testcases/foo
    test_name=${test_file##*/}                    # like foo
    expected_file=${test_base}.txt                # like testcases/foo.txt
...
test_stdout () {
  if diff <($program < ${test_file} 2>/dev/null) ${expected_file} >/dev/null
  then
    let success_count++
    ...
    echo "    The expected output file, \"${expected_file}\" does not match \`$program < ${test_file}\`:" >> $verbose_file
    diff <($program < ${test_file} 2>/dev/null) ${expected_file} 2>&1 |sed 's/^/    /' >> $verbose_file
  fi
}
...
test_error_lines () {
  ...
  then
    ...
    space_out_verbose_file
    echo -e "\033[31mFAILURE ${failure_count}:\033[0m ${test_name}" >> $verbose_file
    echo "    The expected error lines file, \"${expected_erors_file}\" does not match \`$program < ${test_file} 2>&1 >/dev/null\`:" >> $verbose_file
...
if [[ $# -ge 1 ]]; then
  ...
  if which myhtml2txt; then
    program=`which myhtml2txt`
  else
